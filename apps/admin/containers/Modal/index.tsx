  ...
  return createPortal(
    ...
    >
      <div className="flex min-h-screen items-center justify-center p-0 text-center md:block">
        ...
        >
          <header className="border-t-4 border-blue-500 bg-white">
            <div className="flex items-center border-b border-stone-100 px-4 py-3">
              <div className="flex-1">
                <h1 className="text-xl font-semibold">{title}</h1>
                {!!description && (
                  <p className="mt-1 text-sm text-stone-900">{description}</p>
                )}
              </div>
              <button
                onClick={onClose}
          ...
          {footer && (
            <footer className="rounded-b-lg border-t border-neutral-700 bg-white px-7 py-4">
              {footer}
            </footer>
          )}
        </div>
